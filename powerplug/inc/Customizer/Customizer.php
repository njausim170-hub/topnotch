...
final class Customizer implements Bootable {
    ...
    private function register_home( \WP_Customize_Manager $wp ): void {
        ...
        $wp->add_control( 'pp_products_order', array( 'label' => __( 'Product order in homepage rows', 'powerplug' ), 'description' => __( 'Random shows different products on each refresh (recommended).', 'powerplug' ), 'section' => 'pp_home', 'type' => 'select', 'choices' => array( 'rand' => __( 'Random (changes on refresh)', 'powerplug' ), 'date' => __( 'Latest first', 'powerplug' ), 'popularity' => __( 'Best selling', 'powerplug' ), 'price' => __( 'Price: low to high', 'powerplug' ) ) ) );
    }
