...
final class Home {
			...
			<div class="pp-hero__inner">
				<div class="pp-hero__copy">
					...
					<div class="pp-hero__cta">
						...
						<a class="button button-ghost" href="https://wa.me/254708777192" rel="noopener"><?php esc_html_e( 'Chat on WhatsApp', 'powerplug' ); ?></a>
					</div>
				</div>
			</div>
		</section>
    ...
    public static function featured_categories(): void {
        ...
        $terms   = get_terms( array( 'taxonomy' => 'product_cat', 'hide_empty' => true, 'number' => $count, 'orderby' => 'count', 'order' => 'DESC', 'exclude' => array( (int) get_option( 'default_product_cat' ) ) ) );
	...
	public static function shop_by_brand(): void {
		$brands = [ 'Makita', 'Bosch', 'DeWalt', 'Ingco', 'Total', 'Honda' ];
    ...
    public static function hero_row(): void {
        ...
        if ( has_nav_menu( 'hero-categories' ) ) {
            ...
            if ( taxonomy_exists( 'product_cat' ) ) {
                $found = get_terms( array( 'taxonomy' => 'product_cat', 'hide_empty' => true, 'parent' => 0, 'orderby' => 'count', 'order' => 'DESC', 'number' => $cats_count, 'exclude' => array( (int) get_option( 'default_product_cat' ) ) ) );
                if ( is_array( $found ) ) {
                    $terms = $found;
                }
            }
    ...
    public static function category_sections( int $max_cats = 0 ): void {
        ...
        $row_terms   = get_terms( array( 'taxonomy' => 'product_cat', 'hide_empty' => true, 'parent' => 0, 'orderby' => 'count', 'order' => 'DESC', 'number' => $max_cats, 'exclude' => array( $default_cat ) ) );
